...
process_url_button = st.button("Schritt 1: URL verarbeiten", key="process_url")

if process_url_button and url_input:
    markdown_content = convert_url_to_markdown(url_input)
    if markdown_content:
        st.session_state.from_url_current_text = markdown_content
        st.session_state.url = url_input


# If we have text to process
if "from_url_current_text" in st.session_state:
    # Step 1: Extract terms
    if "from_url_extracted_terms" not in st.session_state:
        st.session_state.from_url_extracted_terms = extract_terms_from_text(
            st.session_state.from_url_current_text
        )
        if (
        ...
        terms_text = st.text_area(
            ...
            value="\n".join(st.session_state.from_url_extracted_terms),
            height=200,
            key="terms_editor",
        )
        st.session_state.from_url_extracted_terms = [
        ...
        if generate_button:
            with st.spinner("Erklärungen werden generiert..."):
                ...
                if explanations:
                    st.session_state.from_url_explanations = explanations
                    st.session_state.from_url_explanations_with_context = (
        ...
        if "from_url_explanations" in st.session_state:
            st.success("Erklärungen wurden generiert!")
            st.subheader("Ergebnisse")

            explanations_parsed = [
                (elem.begriff, elem.erklaerung.text)
                for elem in st.session_state.from_url_explanations.begriffe
            ]
            df = pd.DataFrame(
                explanations_parsed, columns=["Begriff", "Erklärung ohne Kontext"]
            )

            if st.session_state.from_url_explanations_with_context:
                explanations_with_context_parsed = [
                    (elem.begriff, elem.erklaerung.text)
                    for elem in st.session_state.from_url_explanations_with_context.begriffe
                ]
            ...
            filename = url_input.split("/")[-1].replace(".html", "")
            if not filename:
                filename = "glossar"
